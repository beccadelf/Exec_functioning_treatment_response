...
task_data <- read.csv(file.path(basic_path,"Performance_Scores/Task_Performance_Scores.csv")) # only for overall accuracy
data_pat_pre <- read.csv(file.path(basic_path,"Data_Patients_Pre.csv"))
```

# 1. Add overall accuracy as additional feature (and PercentRmv if trials with wrong responses were discarded)
```{r}
...
# 2. Encode classes (for classifier) vs. outcome (for regressor)
...
## 2.2. Outcome (Percent-change in FAS)
```{r}
...
# 5. Create feature dataframe without executive task feature for hypothesis testing
...
# 6. Save 2 feature and 1 label dataframes as .csv
