...
task_features <- c("NumberLetter_BIS_Repeat", "NumberLetter_BIS_Switch", 
                   ...
                   "StopSignal_SSRT", "NumberLetter_Overall_PC", "Stroop_Overall_PC", "TwoBack_Overall_PC")
...
socdem_clin_vars <- c("Alter", "Geschlecht", "Abschluss_Gymnasium", "T1_BAT_FAS_score", "T1_BAT_BDI_II_score", "T1_BAT_STAI_T_score","BAT_T1")
features_df <- add_socdem_vars(df = features_df, socdem_file_path = socdem_file_path, vars = socdem_clin_vars)
```

### 1.2. Add response and exclude patients due with missing response
```{r}
...
### 5.3 Create reduced feature set to share model publicly
This model does not contain the BAT as it is often not assessed.
```{r}
discarded_features <- c("BAT", "BIS_11", "Kirby_k", "CFC_14", "SRHI")
clin_features_reduced <- clin_features_df[, !names(clin_features_df) %in% discarded_features]
```
...
write.csv(features_df, file.path(output_path, "all_features_accuracy.csv"), row.names = FALSE)
