...
output:
  html_document:
    ...
    df_print: paged
params:
  response_criterion: "Response_FSQ_50" # choose between "Response_FSQ_50", "FSQ_perc_change", and "Response_BAT" 
---
# Setup
...
#path_results_folder = file.path("Y:\\Psythera\\Projekte_Meinke\\Old_projects\\Labrotation_Rebecca\\4_Make_model_available\\Results\\RT-trimmed-RT-wrong-removed_outliers-removed",
path_results_folder = file.path("Y:\\Psythera\\Projekte_Meinke\\Old_projects\\Labrotation_Rebecca\\4_Make_model_available\\Results", params$response_criterion)
results_folders <- list.files(path = path_results_folder, full.names = FALSE)

# Filter for results-folders
...
collect_p_values <- function(folders_unpermuted, evaluation_metric, logic = "large_better"){
  ...
  for (folder_name in folders_unpermuted) {
    ...
    # Try to load and compare the evaluation metric per iteration
    tryCatch({
...
# Illustration: look at evaluation metrics of a permuted 
```{r}
evaluation_metrics_permuted <- read.delim(file.path("Y:\\PsyThera\\Projekte_Meinke\\Old_projects\\Labrotation_Rebecca\\2_Machine_learning\\Results\\RT-trimmed-RT-wrong-removed_outliers-removed\\response_FSQ_50\\all_features_svm_clf_yes_smote_oversampling_final_permuted", "performance_across_iters.txt"))$balanced_accuracy
hist(evaluation_metrics_permuted)
mean(evaluation_metrics_permuted)
```

# Apply function and show dataframe
```{r}
if (params$response_criterion == "Response_FSQ_50" | params$response_criterion == "Response_BAT"){
  df <- collect_p_values(folders_unpermuted = results_folders, evaluation_metric = "balanced_accuracy")
...
} else if (params$response_criterion == "FSQ_perc_change"){
  df <- collect_p_values(folders_unpermuted = results_folders, evaluation_metric = "MSE", logic = "small_better")

  df$models <- gsub("random_forest_regressor","rf",df$models)
