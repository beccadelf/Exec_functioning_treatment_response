...
generate_filename_out <- function(outliers_removed) {
  outliers_text <- ifelse(outliers_removed == "yes", "outliers_removed", "outliers_not_removed")
...
for (outliers_removed in outliers_removed_options) {
  for (input_data_path in input_data_path_options){
      ...
      output_path = file.path(input_data_path, "BIS", outliers_removed, output_filename)
...
# Group comparison script
input_data_path_group_options = c("Y:\\PsyThera\\Projekte_Meinke\\Old_projects\\Labrotation_Rebecca\\0_Datapreparation\\Daten_Gruppenvergleich\\new\\not_trimmed_not_removed\\BIS\\outliers-not-removed",
...
generate_filename_groupcomp <- function(input_data_path) {
  ...
  paste0("HC_vs_patients_", third_last_folder, "_", last_folder,".html")
}

for (input_data_path in input_data_path_group_options) {
    params_list <- list(input_data_path = input_data_path)
    
    output_filename <- generate_filename_groupcomp(input_data_path)
