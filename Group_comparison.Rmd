...
```
Data for `r nrow(Patients_vs_HC_imp[Patients_vs_HC_imp$Gruppe == 0,])` patients and `r nrow(Patients_vs_HC_imp[Patients_vs_HC_imp$Gruppe == 1,])` healthy controls were available.
...
In the Numberletter task, the difference between patients and healthy controls remained significant when controlling for covariates. However, also education had a main effect on performance.
In the Stroop task, the difference between patients and healthy controls also remained significant when controlling for covariates. Here, also the gender and education had a main effect on performance.
...
# Combine data
Patients_pre_3$Subject <- Patients_pre_3$Subject + 600 # This was the ID given for post
...
```{r}
# Reshape data from wide to long format
Patients_pre_vs_post_imp_long <- Patients_pre_vs_post_imp %>%
  pivot_longer(cols = all_of(BIS_columns_renamed),
               names_to = "Condition",
               values_to = "BIS_Score")

# Add new task variable
Patients_pre_vs_post_imp_long <- Patients_pre_vs_post_imp_long %>%
  mutate(Task = case_when(
    grepl("NumberLetter", Condition) ~ "NumberLetter",
    grepl("Stroop", Condition) ~ "Stroop",
    grepl("TwoBack", Condition) ~ "TwoBack"
