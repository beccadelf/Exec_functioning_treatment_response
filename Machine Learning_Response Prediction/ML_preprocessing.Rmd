...
base_output_path <- create_results_path(inputdata_path = basic_path, output_mainpath = params$output_base_path)
output_path <- file.path(base_output_path, params$response_criterion)
...
if (params$response_criterion == "response_FSQ") {
  ...
  outcome_df <- round(data.frame(FAS_perc_change = ((features_df$BAT_T3 - features_df$BAT_T3) / features_df$BAT_T1) * 100) * (-1),2)
}
```
