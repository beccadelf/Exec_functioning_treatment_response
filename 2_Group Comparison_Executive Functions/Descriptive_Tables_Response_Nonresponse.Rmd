...
data_Pat_pre <- add_socdem_vars(df = data_Pat_pre, socdem_file_path = socdem_file_path, vars = socdem_clin_vars)
```
...
```
We had `r nrow(data_Pat_pre[!is.na(data_Pat_pre$Response),])` patients with a post-value available, and `r nrow(data_Pat_pre_clean[data_Pat_pre_clean$Response == 1,])` responders.
