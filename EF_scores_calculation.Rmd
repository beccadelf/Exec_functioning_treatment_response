...
# 4. Merge task data with data_all
...
# TODO: use merged dataframe instead for following procedures

# 5. Recode patient IDs from post-measurement
...
AllData_TwoBack_merged <- AllData_TwoBack_merged %>%
  mutate(Gruppe = ifelse(Subject >= 216601 & Subject <= 216760, 2, Gruppe))
# SST
AllData_SST_merged <- AllData_SST_merged %>%
  mutate(Gruppe = ifelse(Subject >= 216601 & Subject <= 216760, 2, Gruppe))
```

# 6. Exclusion of subjects for each task individually, separate HC from patients
...
## Stop-Signal Task
```{r Subject exclusion SST}

# Remove subjects with missing group information 
AllData_SST_clean.1 <- AllData_SST_merged %>%
  filter(!is.na(Gruppe))
excl_subs_2B_group <- anti_join(AllData_SST_merged, AllData_SST_clean.1, by = "Subject")

# Remove patients with missing FAS_T1 and/or FAS_T3
AllData_SST_clean.2 <- AllData_SST_clean.1 %>%
  filter(Gruppe == 0 | Gruppe == 2 | (Gruppe == 1 & !is.na(T1_BAT_FAS_score) & !is.na(T3_BAT_FAS_score)))
excl_subs_2B_rc <- anti_join(AllData_SST_clean.1, AllData_SST_clean.2, by = "Subject")
...
# 7. Calculate BIS (and Difference Score) for each condition and subject separately
BIS = Z(PC) - Z(RT)
...
# 8. Create separate dataframe for each group (HC, patient_pre, patient_post)
