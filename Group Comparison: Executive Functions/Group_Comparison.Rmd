...
library(car)
library(extrafont)
font_import(path = "C:/Windows/Fonts", pattern = "arial")  # Import Arial fonts => only needs to be done once to load plot
...
ggsave("C:/Users/Till/Desktop/violin_plots_patients_hc.png", plot = violin_plots_patients_hc, width = 10, height = 8, dpi = 300)
```
...
```{r}
# Prepare data for NumberLetter, Stroop, and TwoBack
Response_vs_NonResponse_long <- data_Pat_pre %>%
  filter(!is.na(Response)) %>%  # Exclude rows where Response is NA
  pivot_longer(cols = all_of(imp_columns[-length(imp_columns)]),  # Exclude SSRT from this data frame
               names_to = "Condition",
               values_to = "BIS_Score") %>%
  mutate(Task = case_when(
    grepl("NumberLetter", Condition) ~ "NumberLetter",
  ...
  mutate(Condition = gsub("NumberLetter_BIS_|Stroop_BIS_|TwoBack_BIS_", "", Condition)) %>%
  filter(!is.na(BIS_Score))  # Filter out NAs for these tasks only

# Prepare data for SSRT as a separate dataset
Response_vs_NonResponse_SSRT <- data_Pat_pre %>%
  filter(!is.na(Response)) %>%  # Exclude rows where Response is NA
  select(Subject, Response, SSRT) %>%
  rename(BIS_Score = SSRT) %>%  # Rename for consistency with other tasks
  mutate(Task = "SSRT", Condition = "SSRT") %>%
  filter(!is.na(BIS_Score))  # Filter out NAs for SSRT only

# Convert "Response" to factor in both datasets
Response_vs_NonResponse_long$Response <- factor(Response_vs_NonResponse_long$Response, levels = c(0, 1), labels = c("Non-Responder", "Responder"))
Response_vs_NonResponse_SSRT$Response <- factor(Response_vs_NonResponse_SSRT$Response, levels = c(0, 1), labels = c("Non-Responder", "Responder"))
```

```{r}
# Create violin plot for NumberLetter, Stroop, and TwoBack
violin_plots_response <- ggplot(Response_vs_NonResponse_long, aes(x = Condition, y = BIS_Score, fill = Response)) +
  geom_violin(position = position_dodge(width = 0.8)) +
  stat_summary(fun.data = mean_sdl, geom = "pointrange", 
               position = position_dodge(width = 0.8)) +
  facet_wrap(~ Task, scales = "free_x") +
  labs(title = "Violin Plots of Task Condition by Response",
       x = "Task Condition",
       y = "BIS-Score",
       fill = "Group") +
  theme(legend.position = "none")  # Remove legend for combined display

# Create separate violin plot for SSRT with adjusted scale if needed
ssrt_violin_plot_response <- ggplot(Response_vs_NonResponse_SSRT, aes(x = Condition, y = BIS_Score, fill = Response)) +
  geom_violin(position = position_dodge(width = 0.8)) +
  stat_summary(fun.data = mean_sdl, geom = "pointrange", 
               position = position_dodge(width = 0.8)) +
  labs(x = NULL, y = NULL) +  # Remove x-axis and y-axis labels
  theme(plot.title = element_blank(),  # Remove the standalone title
        axis.ticks.x = element_blank())

# Combine plots with patchwork, adjusting the widths to balance layout
combined_plot_response <- violin_plots_response + ssrt_violin_plot_response + 
  plot_layout(ncol = 2, widths = c(9, 1))  # Adjust widths to give more space to the left plot

# Display the combined plot
print(combined_plot_response)
