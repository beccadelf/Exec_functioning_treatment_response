...
rename_map <- c(
  ...
  'SSRT' = "Stop-Signal RT")
t_test_table_pub$`Performance Measure` <- rename_map[t_test_table_pub$`Performance Measure`]
```
...
# Convert rownames (dependent variables) to separate column
chi_sq_test_descr_pub <- cbind(dependent_variables = rownames(chi_sq_test_descr), chi_sq_test_descr)

# Change colnames
colnames(chi_sq_test_descr_pub) <- c("Categorical variables", "Test Type", "Chi-Square-Statistic", "df", "p-Value", "adj. p-Value")
```
