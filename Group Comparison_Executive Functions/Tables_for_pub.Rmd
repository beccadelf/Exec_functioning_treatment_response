...
descr_comps_pub <- rbind(t_test_descr_pub, chi_sq_test_descr_pub)
# Reorder and rename the variables
descr_comps_pub <- reorder_and_rename_rows(df = descr_comps_pub, col_name = "Variable", label_map = variable_labels)

# Print missings
for (i in seq_len(nrow(descr_comps_pub))) {
  miss_hc <- descr_comps_pub$Missings_HC[i]
  miss_pat <- descr_comps_pub$Missings_Patients[i]
  
  if (miss_hc > 0 || miss_pat > 0) {
    var_name <- descr_comps_pub$Variable[i]
...
descr_comps_pub <- descr_comps_pub[, !(names(descr_comps_pub) %in% c("Missings_HC", "Missings_Patients"))]
...
descr_comps_pub <- rbind(n_subjects_row, descr_comps_pub)
```

### 3. Create and save flextable
```{r}
create_save_flextable(table_pub = descr_comps_pub, results_path = results_path, file_name = "descr_tests_Patients_HCs.docx")
```


# Descriptive Statistics

### 0. Function to generate characteristics table
```{r}
generate_characteristics_table <- function(data, task_name, grouping_variable, group_labels) {
  data %>%
    ...
    mutate(grouping_variable = factor(Gruppe, levels = c(0, 1), labels = group_labels),
           ...
           Abschluss = factor(Abschluss, levels = c(1, 2, 3, 4), 
                              labels = c("Basic secondary education", 
                                         ...
                                         "Other"))) %>%
    tbl_summary(by = grouping_variable,  # Compare by Gruppe (HC vs. Pat),
