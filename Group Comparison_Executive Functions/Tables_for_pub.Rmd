  ...
  if (type == "dimensional") {
    ...
    t_test_table_pub <- t_test_table_pub %>%
      mutate(
        ...
        `Comparison` = paste0("χ²(", df, ") = ", statistic, ", p = ", p_value_adjusted)
      ) %>%
      select(dependent_variables, `Healthy Controls`, `Patients`, `Comparison`)
  }
...
desired_order <- desired_order[desired_order %in% descr_comps_pub$Variable]
#descr_comps_pub <- descr_comps_pub[match(desired_order, descr_comps_pub$Variable), ]
#descr_comps_pub$Variable <- variable_labels[descr_comps_pub$Variable]
```
