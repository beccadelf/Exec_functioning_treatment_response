...
```{r}
task_performance_path = "Y:/PsyThera/Projekte_Meinke/Old_projects/Labrotation_Rebecca/0_Datapreparation/Daten_Gruppenvergleich/With_wrong_responses"
...
# Descriptive statistics for confounds
```{r}
...
# TODO: Direkt als word speichern
# TODO: Was sind die Unknown-Werte? Die rausnehmen und Zahlen aktualisieren! Abschluss umkodieren (1,2,3,4)
```

# Testing assumptions for ANCOVA
## Linearity between age and performance
Sex and education are categorical variables; ergo, only age will be tested for linearity.
This is done by controlling performance for sex, education, and group, and plotting the residuals against age.
### in general
```{r}
...
for (task in performance_tasks) {
  ...
  p <- ggplot(model_data, aes(x = Alter, y = residuals)) +
    ...
    geom_smooth(method = "loess", se = TRUE, color = "blue") +  # Lowess Fit Line
...
### spllited by groups
```{r}
# Loop through each task performance variable and plot Task Performance~Age within groups (HC vs. Pat)
for (task in performance_tasks) {
...
In order to test for homoscedasticity, we employ Levene's test for the categorical (group comparison) covariates sex and education and Breusch-Pagan test for continous covariates (age)
```{r}
...
## Non-interaction / homogeneity of regression slopes
We cannot test homogeneity of slopes for the covariates of sex and education. 
Instead, we test if there is a significant interaction between these covariates and the group-variable (patients vs. healthy controls). 
...
for (i in seq_along(performance_tasks)) {
  ...
  # 1. Gender; Model to test the interaction between Gender and Group, controlled for Age and Education
  ...
  # 2. Abschluss: Model to test the interaction between Group and Education, controlled for Gender and Age
  ...
  # 3. Age: Model to test the homogeneity of slopes for Age, controlled for Gender and Education
...
<u>Additional: Difference between Patients and Healthy Control groups in the covariates (indipendence)</u>
```{r}
#Calculate t-test to test for difference in age between Pat vs HC
t_test_age <- t.test(Alter ~ Gruppe, data = Patients_vs_HC_imp_conf)
mean_age_pat <- t_test_age$estimate[2]
mean_age_hc <- t_test_age$estimate[1]

#2x2 table with Female/Male | Pat/HC tested for independence
contingency_table_gender <- table(Patients_vs_HC_imp_conf$Geschlecht, Patients_vs_HC_imp_conf$Gruppe)
chi_square_test_gender <- chisq.test(contingency_table_gender)

#Contingency table with 4 levels of education and Pat/HC
contingency_table_education <- table(Patients_vs_HC_imp_conf$Abschluss, Patients_vs_HC_imp_conf$Gruppe)
fisher_test_education <- fisher.test(contingency_table_education) # Fisher because some cells have <5 values (Chi-squared approximation may be imprecise)

# Extract p-values from each test
age_p_value <- t_test_age$p.value
gender_p_value <- chi_square_test_gender$p.value
education_p_value <- fisher_test_education$p.value

# Create a data frame with the covariates and their corresponding p-values
p_value_table <- data.frame(
  Covariate = c("Age", "Sex", "Education"),
  `p-value` = c(age_p_value, gender_p_value, education_p_value)
)

pander::pander(p_value_table, style = "rmarkdown")

# average age of Pat and HC groups
average_age_table <- data.frame(
  Group = c("Patients", "Healthy Controls"),
  `Average Age` = c(mean_age_pat, mean_age_hc)
)
cat("\n**Average Age of Groups**\n")
pander::pander(average_age_table, style = "rmarkdown")

# contingency table for gender (Male/Female | Pat/HC)
cat("\n**Contingency Table: Gender (Male/Female) vs. Group (Pat/HC)**\n")
pander::pander(contingency_table_gender, style = "rmarkdown")

# contingency table for education levels vs. group
cat("\n**Contingency Table: Education Levels vs. Group (Pat/HC)**\n")
pander::pander(contingency_table_education, style = "rmarkdown")
```

# Comprehensive ANCOVA
```{r}
# Modified function to include corrected effect size (partial eta-squared) and power
ANCOVA_mult_cols <- function(df_basis, cols, homoskedasticity_violations, interaction_violations, covariates){
  ...
  df <- data.frame(
    ...
    Alter_p_value = numeric(length(cols)),
    Geschlecht_p_value = numeric(length(cols)),
    Abschluss_p_value = numeric(length(cols)),
    eta_squared = numeric(length(cols)),          # effect size (partial eta-squared)
    power = numeric(length(cols)),
    Assumption_Violation = character(length(cols)),  # violation status
    stringsAsFactors = FALSE
  )
  rownames(df) <- cols
  ...
  for (i in seq_along(cols)){
    ...
    if (interaction_violation == "Violated" && homoskedasticity_violation == "Violated") {
      # 2.1 Both violations: Apply robust standard errors and model interaction
      ...
      # eta-squared and power
      SS_group_corrected <- sum(robust_summary["Gruppe", "t value"]^2)  # Estimation of corrected SS
      SS_residual_corrected <- sum(robust_summary[,"t value"]^2) - SS_group_corrected
      eta_squared_corrected <- SS_group_corrected / (SS_group_corrected + SS_residual_corrected)
      df[col, "eta_squared"] <- round(eta_squared_corrected, 2)
      
      f_squared_corrected <- eta_squared_corrected / (1 - eta_squared_corrected)
      power_corrected_result <- pwr.anova.test(k = 2, n = nrow(df_basis), f = sqrt(f_squared_corrected), sig.level = 0.05)
      df[col, "power"] <- round(power_corrected_result$power, 2)
      
      df[col, "Assumption_Violation"] <- "Heteroscedasticity and Interaction"
      
    } else if (interaction_violation == "Violated") {
      # 2.2 Only interaction violated: Model the interaction
      formula <- paste(col, "~ Gruppe * (", covariate_formula, ")")
      ...
      # eta-squared and power
      SS_group_corrected <- summary_model[[1]]$"Sum Sq"[1]
      SS_residual_corrected <- summary_model[[1]]$"Sum Sq"[length(summary_model[[1]]$"Sum Sq")]
      eta_squared_corrected <- SS_group_corrected / (SS_group_corrected + SS_residual_corrected)
      df[col, "eta_squared"] <- round(eta_squared_corrected, 2)
      ...
      # 2.3 Only homoscedasticity violated: Apply robust standard errors
      formula <- paste(col, "~ Gruppe +", covariate_formula)
      ...
      # Calculate eta-squared and power for uncorrected model
      SS_group <- summary_ancova[[1]]$"Sum Sq"[1]
      SS_residual <- summary_ancova[[1]]$"Sum Sq"[length(summary_ancova[[1]]$"Sum Sq")]
      eta_squared <- SS_group / (SS_group + SS_residual)
      df[col, "eta_squared"] <- round(eta_squared, 2)
      
      f_squared <- eta_squared / (1 - eta_squared)
      power_result <- pwr.anova.test(k = 2, n = nrow(df_basis), f = sqrt(f_squared), sig.level = 0.05)
      df[col, "power"] <- round(power_result$power, 2)
