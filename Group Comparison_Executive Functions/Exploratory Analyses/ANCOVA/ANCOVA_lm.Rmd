...
### 3. Descriptive statistics for covariates
```{r}
...
### Assessing assumptions statistically
#### Normality
```{r}
...
```{r}
tasks_GLMviolated <- c("NumberLetter_BIS_Repeat", # visual and statistical violation of homogeneity 
...
# Define models 
tasks_GLMviolated_add_interaction <- interaction_results[interaction_results$Interaction_Group_Sex_p_value
...
tasks_GLMviolated_add_nointeraction <- setdiff(tasks_GLMviolated, tasks_GLMviolated_add_interaction)
...
## 3. Rerun models with interaction terms
```{r, fig.width = 10, message=FALSE}
results_corrected_interaction <- list()
for (task in tasks_GLMviolated_add_interaction) {
...
```
The visualizations shows that homoscedasticity is not violated anymore in Number_Letter_BIS_Switch but remains violated in TwoBack_BIS_Target.


## 4. Rerun models with correction for heteroscedasticity
### Define models to rerun with correction for heteroscedasticity
Based on visual inspection, the model for TwoBack_BIS_Target should also be corrected for heteroscedasticity.
```{r}
tasks_GLMviolated_correct_homogeneity <- c(tasks_GLMviolated_add_nointeraction, "TwoBack_BIS_Target")
```

### Rerun models with correction
```{r, message=FALSE}

results_corrected_homogeneity <- list()
for (task in tasks_GLMviolated_correct_homogeneity) {
