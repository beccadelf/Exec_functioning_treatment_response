...
# ANCOVA specific functions
```{r}
ANCOVA_mult_cols_no_violation <- function(df_basis, cols, covariates){
  
  # Build the covariates effects for the formula
  covariate_main_effects <- paste(covariates, collapse = " + ")
  
  # Perform the uncorrected ANCOVA
  ...
  df <- summarize_ancova_results(results_list = results_uncorrected)
  
  return(df)
}

ANCOVA_mult_cols <- function(df_basis, cols, covariates, homoskedasticity_violations, interaction_violations){
  
  # Build the covariates effects for the formula
  covariate_main_effects <- paste(covariates, collapse = " + ")
  covariate_interaction_effects <- paste(paste("Gruppe",covariates, sep = ":"), collapse = " + ")
  ...
  for (i in seq_along(cols)){
    ...
    # 1. ANCOVA (uncorrected) for every executive functioning measure
    formula_ancova <- paste(col, "~ Gruppe +", covariate_main_effects)
    results_ancova[[col]] <- run_ancova(data = df_basis,formula = formula_ancova, heterosced = FALSE)
    results_ancova[[col]][["condition"]] <- "none"
  }
    
  # Perform a corrected ANCOVA if violations of assumptions occurred
  for (i in seq_along(cols)){
    col <- cols[i]
    interaction_violation <- interaction_violations[i]
    homoskedasticity_violation <- homoskedasticity_violations[i]
    
    # Needed for the violation of noninteraction
    formula_ancova_interact <- paste(col, "~ Gruppe +", covariate_main_effects, "+", covariate_interaction_effects)
    
    # Both non-interaction-assumption and homoskedasticity-assumption are violated
    if (interaction_violation == "Violated" && homoskedasticity_violation == "Violated") {
      results_ancova[[col]]<- run_ancova(formula = formula_ancova_interact, data = df_basis, heterosced = TRUE)
      results_ancova[[col]][["condition"]] <- "both"
      # Add sums of squared error as NA 
      results_ancova[[col]][["anova_results_sign"]][["Sum Sq"]] <- rep(NA_real_)

    #  Only non-interaction-assumption violated: Model the interaction
    } else if (interaction_violation == "Violated") {
      results_ancova[[col]]<- run_ancova(formula = formula_ancova_interact, data = df_basis, heterosced = FALSE)
      results_ancova[[col]][["condition"]] <- "noninteraction"

    # Only homoskedasticity-assumption violated: Apply robust estimator of variation
    } else if (homoskedasticity_violation == "Violated") {
      results_ancova[[col]] <- run_ancova(formula = formula_ancova_interact, data = df_basis, heterosced = TRUE)
      results_ancova[[col]][["condition"]] <- "heteroscedasticity"
       # Add sums of squared error as NA 
      results_ancova[[col]][["anova_results_sign"]][["Sum Sq"]] <- rep(NA_real_)
    }
  }
  ...
  # Summarize results
  df <- summarize_ancova_results(results_list = results_ancova)
...
```{r}
tasks_GLMviolated <- c("NumberLetter_BIS_Repeat","NumberLetter_BIS_Switch","Stroop_BIS_Congruent","NumberLetter_BIS_Diff_Score", "Stroop_BIS_Incongruent",
                       "TwoBack_BIS_Target")
```
...
### 3. Rerun models wiht interaction terms
```{r, fig.width = 10}
...
# 4. Rerun models for those that did not improve with correction for heteroscedasticity
```{r}
...
for (task in tasks_no_improvement) {
  # 1. ANCOVA (uncorrected) for every executive functioning measure
...
summarize_ancova_results(results_list = combined_results)

summarize_ancova_results(results_list = results_uncorrected)
```
...
## Corrected for violations if needed
```{r}


table_ANCOVA <- ANCOVA_mult_cols_no_violation(df_basis = Patients_vs_HC_imp_conf_final,
                                cols = performance_tasks,
                                covariates = c("Alter", "Geschlecht", "Abschluss_Gymnasium"))
