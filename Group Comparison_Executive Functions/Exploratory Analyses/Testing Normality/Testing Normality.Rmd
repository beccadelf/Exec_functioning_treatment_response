...
## #Manual Range
```{r}
create_density_plot <- function(data, col) {
  group_data <- data[, c("Gruppe", col)]
  
  ggplot(group_data, aes(x = get(col), fill = as.factor(Gruppe))) +
    geom_density(alpha = 0.5) +
    labs(title = col,
         x = col,
         y = "Density",
         fill = "Group") +
    scale_fill_manual(values = c("blue", "red"), labels = c("HC", "Patients")) +
    facet_wrap(~ Gruppe, scales = "free_x", labeller = as_labeller(c("0" = "HC", "1" = "Patients"))) +
...
generate_density_plots <- function(df_basis, cols) {
  plots <- list()
  for (col in cols) {
    plots[[col]] <- create_density_plot(df_basis, col)
  }
  return(plots)
}
...
```{r}
#automatically takes min and max values and adjusts value range
...
create_density_plot <- function(data, col, x_range) {
  ggplot(data, aes(x = get(col), fill = as.factor(Gruppe))) +
    geom_density(alpha = 0.5) +
    labs(title = col,
         x = col,
         y = "Density",
...
generate_density_plots <- function(df_basis, cols, x_range) {
  plots <- list()
  for (col in cols) {
    plots[[col]] <- create_density_plot(df_basis, col, x_range)
  }
  return(plots)
}
...
```{r}
#Individual x-axis for each group according to min and max values of that task
create_density_plot <- function(data, col) {
  x_range <- range(data[, col], na.rm = TRUE) # By each group min-max
