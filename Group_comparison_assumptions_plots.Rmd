...
t_test_mult_cols <- function(df_basis, cols) {
  ...
  for (col in cols) {
    ...
    n1 <- length(group0)
    n2 <- length (group1)
    power_result <- pwr.t.test(d = cohen_d, n = min(n1, n2), sig.level = 0.05, type = "two.sample", alternative = "greater")
...
# Apply the K-S test to the difference values in your data
...
paired_t_test_mult_cols <- function(df_basis, cols) {
  ...
  for (col in cols) {
    ...
    # non-formula interface as above
